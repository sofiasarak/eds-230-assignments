...
```{r}
alm_yield <- function(clim){
  
  # Find monthly temperature and precipitation values
  climate_month <- clim %>% 
  group_by(year, month) %>% 
  summarize(tmin_month = mean(tmin_c, na.rm = TRUE),
            tmax_month = mean(tmax_c, na.rm = TRUE),
            precip_month = mean(precip, na.rm = TRUE))
  
  # Save necessary variables from df
  tmin_2 <- climate_month[climate_month$month == 2, ][["tmin_c"]]
  prec_1 <- climate_month[climate_month$month == 1, ][[ "precip"]]
  
  # Apply formula to variables
  yield <- (-0.015 * tmin_2) - (0.0046 * (tmin_2)^2) - (0.07 * prec_1) + (0.0043 * (prec_1)^2) + 0.28
...
```{r}
 # Find monthly temperature and precipitation values
  ...
  # Save necessary variables from df
  tmin_2 <- climate[climate$month == 2, "tmin_c"]
  prec_1 <- climate[climate$month == 1, "precip"]
