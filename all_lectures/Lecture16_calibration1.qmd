...
# find the parameter set with the best result (best match to available data)
best_par_nse = which.max(res)
...
# changing your metrics/time frames may change which parameter set is the best
```
...
rmsq <- function(m, o) {
  ...
  mean_err_sq <- mean(err_sq)
  
  # take the square root
  sqrt_mean_err_sq <- sqrt(mean_err_sq)
  return(sqrt_mean_err_sq)
}

res = msage %>% select(-date, -month, -day, -year, -wy, -obs ) %>% 
  map_dbl(rmsq, o=msage$obs)

best_par_rmsq = which.max(res)
...
res = july %>% select(-date, -month, -day, -year, -wy, -obs ) %>% map_dbl(nse_adj, o=msage$obs)
