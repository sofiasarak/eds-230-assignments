...
# try another year
ggplot(subset(msagel, wy == 1985), # subset a specific year from the data
...
#alternatively
param_runs = msage %>% 
  select(-date, -month, -day, -year, -wy, -obs ) # keep only the model results, those are argument m
res = map_dbl(.x=param_runs, .f=nse, o=msage$obs) # argument o is the actual observations

head(res)
summary(res)

# basic calibration would be picking a single parameters set; could use min if you had a function that was minimizing something, for example
...
# pick out only the values in the 3rd quartile
res_thresh = summary(res)["3rd Qu."]

# assign acceptable results to only those in the third quartile
accept_res = res[which(res > res_thresh)]
pres + geom_boxplot(data=as.data.frame(accept_res), aes(y=accept_res), col="red")

# pulls out quantities from results
quantile(res, probs = c(0,0.25,0.5,0.75,1))

# prints the quantiles from the line above
summary(res)
...
res2 = msage_month %>%
  ...
  #explicitly give the function the observations
