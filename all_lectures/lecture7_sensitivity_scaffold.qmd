...
estimate <- map(f, ~simple_erosion(I=2, f =.x)) %>% 
  unlist()
# estimate returns list of zeros, meaning on average we won't get any runoff
...
source(here("compute_almond_yield.R"))
...
parm[, "Tmincoeff2"] <- qnorm(parm_quant[,"Tmincoeff2"], mean = -0.0846, sd = 0.0846/pvar)
parm[, "Pcoeff1"] <- qnorm(parm_quant[,"Pcoeff1"], mean = -0.07, sd = 0.07/pvar)
parm[, "Pcoeff2"] <- qnorm(parm_quant[,"Pcoeff2"], mean = -0.0043, sd = 0.004/pvar)
parm[, "intercept"] <- qunif(parm_quant[,"intercept"], min=0.28-0.28*pvar, max = 0.28+0.28 * pvar)
...
clim <- read.table(here("week 3/clim.txt"))
...
## SOFIA: I went wrong somewhere. whatever.
...
ggplot(yieldsd, aes(maxyield)) +
  stat_ecdf() # tells you max yield values; linear suggests even distribution
...
tmp2 <- tmp %>% 
  gather(maxyield, minyield, minyield, value = "yvalue", key = "yieldtype")
